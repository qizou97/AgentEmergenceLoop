...
Test 2 (feasible path — skipped explicitly if entry-point cannot run):
  ...
  If a future environment ships the required deps, remove the skip and assert:
    - blocker.blocked is False
    - execution_log.status in {"success", "failed"}
    - real stdout_excerpt / stderr_excerpt captured
    - both files exist
...
def test_s09_feasible_path_runs_real_subprocess(tmp_path):
    ...
    absent.  If ever enabled: build a data_manifest where all required items
    are available:true (pointing at genuinely existing real-task files under
    data/), zero blocker_risk_ids, and assert blocker.blocked:false,
    execution_log.status in {"success","failed"}, real stdout/stderr captured.
    """
    from sobench.steps import s09_execute_or_block

    # This body is unreachable while the skip is active.
    # When re-enabling: construct artifacts pointing at real, existing data
    # and assert the feasible-path behavior below.

    ws = Workspace(REAL_TASK, REAL_METHOD, REAL_CASE, root=str(tmp_path / "workspaces"))
    ws.dir.mkdir(parents=True, exist_ok=True)

    # Placeholder: supply real artifacts with available:true required items
    # and verify both files are written with the feasible-path shapes.
    s09_execute_or_block.run(ws)
    ...
    # stdout_excerpt and stderr_excerpt are strings (may be empty if command silent)
