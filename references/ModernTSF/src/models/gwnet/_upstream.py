...
* Removed any reliance on hardcoded CUDA devices. The graph-convolution
  operator already moves each support adjacency onto the input tensor's
  device via ``a.to(x.device)``; the learned node embeddings are
  ``nn.Parameter`` so they follow the module's device. No tensor is created
  on a fixed device.
* The public ``forward`` keeps the BasicTS signature
  ``forward(history_data, future_data, batch_seen, epoch, train, **kwargs)``
  with ``history_data`` of shape ``(B, L, N, C)`` and output ``(B, L, N, 1)``.
"""
