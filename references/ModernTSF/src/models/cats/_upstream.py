...
class CATS(nn.Module):
    def __init__(
        ...
        self.backbone = Dummy_Embedding(
            c_in, seq_patch_num=seq_patch_num, patch_len=patch_len,
            pred_patch_num=pred_patch_num, n_layers=n_layers, d_model=d_model,
            n_heads=n_heads, d_ff=d_ff, attn_dropout=attn_dropout,
            dropout=dropout, QAM_start=QAM_start, QAM_end=QAM_end,
            res_attention=res_attention, independence=independence,
    ...
    def forward(self, z, label=None):
        ...
        # z: [bs x nvars x seq_patch_num x patch_len]

        z = self.backbone(z)
        # z: [bs x nvars x pred_patch_num x d_model]

        z = self.proj(z)
        # z: [bs x nvars x pred_len]

        # denorm
        z = z * std
        z = z + mean

        z = z.transpose(1, -1).unsqueeze(-1)
        return z  # (B, horizon, N, 1)
...
class Dummy_Embedding(nn.Module):
    def __init__(self, c_in, seq_patch_num, patch_len, pred_patch_num,
        ...
        self.decoder = Decoder(
            ...
            QAM_end=QAM_end, res_attention=res_attention, n_layers=n_layers,
            store_attn=store_attn)

    def forward(self, x) -> Tensor:
        # x: [bs x nvars x seq_patch_num x patch_len]
        bs = x.shape[0]
        n_vars = x.shape[1]
        # Input encoding
        ...
        if self.independence:
            ...
            pred_patch = torch.reshape(
                pred_patch, (bs * n_vars, pred_patch.shape[2], pred_patch.shape[3]))
...
class Decoder(nn.Module):
    def __init__(self, seq_patch_num, d_model, n_heads, pred_patch_num,
                 d_ff=None, attn_dropout=0., dropout=0., QAM_start=0.1,
                 QAM_end=0.5, res_attention=False, n_layers=1, store_attn=False):
        super().__init__()
        self.layers = nn.ModuleList([
            DecoderLayer(
                seq_patch_num, d_model, pred_patch_num, n_heads=n_heads,
                d_ff=d_ff, QAM_start=QAM_start, QAM_end=QAM_end,
                attn_dropout=attn_dropout, dropout=dropout,
                res_attention=res_attention, store_attn=store_attn)
            for i in range(n_layers)
        ])
        self.res_attention = res_attention

    def forward(self, seq_patch: Tensor, pred_patch: Tensor):
        scores = None
        if self.res_attention:
            for mod in self.layers:
                seq_patch, pred_patch, scores = mod(seq_patch, pred_patch, prev=scores)
            return pred_patch
        else:
            for mod in self.layers:
                seq_patch, pred_patch = mod(seq_patch, pred_patch)
            return pred_patch


class DecoderLayer(nn.Module):
    def __init__(self, seq_patch_num, d_model, pred_patch_num, n_heads,
                 d_ff=256, store_attn=False, QAM_start=0.1, QAM_end=0.5,
        ...
        # Add & Norm
        self.dropout_attn = QueryAdaptiveMasking(
            dim=1, start_prob=QAM_start, end_prob=QAM_end)
        ...
        # Add & Norm
        self.dropout_ffn = QueryAdaptiveMasking(
            dim=1, start_prob=QAM_start, end_prob=QAM_end)
...
class _MultiheadAttention(nn.Module):
    def __init__(self, d_model, n_heads, res_attention=False,
                 attn_dropout=0., proj_dropout=0., qkv_bias=True):
        super().__init__()
        d_h = d_model // n_heads
        self.scale = d_h ** -0.5
        ...
        self.to_out = nn.Sequential(
            nn.Linear(n_heads * d_h, d_model), nn.Dropout(proj_dropout))
