"""Vendored MTGNN architecture and layers.

Vendored/adapted from https://github.com/GestaltCogTeam/BasicTS
(baselines/MTGNN), Apache-2.0. Original model:
"Connecting the Dots: Multivariate Time Series Forecasting with Graph
Neural Networks" (Wu et al., SIGKDD 2020), https://arxiv.org/abs/2005.11650.

Adjustments for ModernTSF:
* merged ``mtgnn_arch.py`` + ``mtgnn_layers.py`` into one module;
* ``self.idx`` is a registered buffer so it tracks the model's device;
* all internally-created tensors derive their device from the input
  (no hardcoded ``cuda`` / ``.to('cuda')``).
"""
...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx):
        ...
        mask.fill_(float("0"))
        s1, t1 = adj.topk(min(self.k, idx.size(0)), 1)
...
class MTGNN(nn.Module):
    ...
    Paper: Connecting the Dots (Wu et al., SIGKDD 2020).
    ...
    def __init__(
        ...
        self.dropout = dropout
        # Predefined adjacency registered as a buffer so it follows the device.
        ...
        # Registered as a buffer so it tracks the model device.
        self.register_buffer("idx", torch.arange(self.num_nodes))

    def forward(self, history_data: torch.Tensor, idx=None, **kwargs) -> torch.Tensor:
        """Feed-forward of MTGNN.

        Args:
            history_data (torch.Tensor): history with shape [B, L, N, C].
            idx (Tensor, optional): graph-learning node index. Defaults to None.

        Returns:
            torch.Tensor: prediction with shape [B, out_dim, N, 1].
        """
