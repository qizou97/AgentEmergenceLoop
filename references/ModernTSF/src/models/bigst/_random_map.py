"""Random feature map helpers for BigST's linearized convolution.

Vendored/adapted from https://github.com/GestaltCogTeam/BasicTS
(baselines/BigST/arch/random_map.py), Apache-2.0. The only change is the
missing ``import numpy as np`` (the upstream file used ``np`` without
importing it; the ``struct_mode`` branch was simply never exercised).
"""
...
def random_feature_map(data, is_query, projection_matrix=None, numerical_stabilizer=0.000001):
    ...
    if is_query:
        data_dash = ratio * (
            torch.exp(data_dash - diag_data - torch.max(data_dash, dim=last_dims_t, keepdim=True)[0])
