"""Vendored DFDGCN architecture.

Vendored/adapted from https://github.com/GestaltCogTeam/DFDGCN
(``DFDGCN/basicts/archs/arch_zoo/dfdgcn_arch/dfdgcn_arch.py``), the official
implementation by the BasicTS authors (GestaltCogTeam, MIT licensed).

DFDGCN ("Dynamic Frequency Domain Graph Convolution Network") builds on Graph
WaveNet: a stack of dilated temporal convolutions plus graph convolution that
mixes (a) predefined adjacencies, (b) a learned adaptive adjacency, and (c) a
*dynamic* adjacency derived per-batch from the FFT magnitude spectrum of the
traffic series, combined with node-identity and time-of-day / day-of-week
embeddings.

Local edits vs. upstream:

* Device-safe integer indexing: ``.type(torch.LongTensor)`` (which forces CPU
  and would mismatch parameters on GPU) is replaced with ``.long()``.
* Calendar index safety: time-of-day / day-of-week embedding lookups are
  clamped to the valid table range so that normalized ``[0, 1)`` calendar
  covariates (ModernTSF convention) do not index out of bounds.
"""
    ...
    def forward(self, x, A, dims):
        if dims == 2:
            ...
            raise NotImplementedError(
                "DFDGCN not implemented for A of dimension " + str(dims)
            )
        return x.contiguous()
...
class DFDGCN(nn.Module):
    def __init__(
        ...
        self.Ex1 = nn.Parameter(
            torch.randn(self.fft_len, self.emb), requires_grad=True
        )
        self.Wd = nn.Parameter(
            torch.randn(
                num_nodes,
                self.emb + self.identity_emb + self.seq_len * 2,
                self.hidden_emb,
            ),
            requires_grad=True,
        )
        self.Wxabs = nn.Parameter(
            torch.randn(self.hidden_emb, self.hidden_emb), requires_grad=True
        )

        self.mlp = linear(residual_channels * 4, residual_channels)
        self.layersnorm = torch.nn.LayerNorm(
            normalized_shape=[num_nodes, self.hidden_emb],
            eps=1e-08,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.gcn_bool and self.addaptadj and self.supports is not None:
            ...
            # Device-safe, range-safe calendar indexing. ModernTSF supplies
            # time-of-day / day-of-week in [0, 1); upstream BasicTS supplies
            # tod in [0, 1) and dow as integers 0..6. Clamp to the table range.
            tod_idx = (data[:, :, :, 1] * 288).long().clamp_(0, 287)
            dow_raw = data[:, :, :, 2]
            # If day-of-week looks normalized ([0, 1)), scale up to 0..6.
            dow_idx = torch.where(
                dow_raw.max() <= 1.0, dow_raw * 7.0, dow_raw
            ).long().clamp_(0, 6)
            T_D = self.T_i_D_emb[tod_idx][:, -1, :, :]
            D_W = self.D_i_W_emb[dow_idx][:, -1, :, :]

            xn1 = torch.fft.rfft(xn1, dim=-1)
            xn1 = torch.abs(xn1)

            xn1 = torch.nn.functional.normalize(
                xn1, p=2.0, dim=1, eps=1e-12, out=None
            )
            xn1 = (
                torch.nn.functional.normalize(xn1, p=2.0, dim=2, eps=1e-12, out=None)
                * self.a
            )
