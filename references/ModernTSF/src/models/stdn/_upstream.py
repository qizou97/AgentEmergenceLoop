...
* Dropped the unused ``torch_geometric`` imports and the ``GCN`` class that
  depended on them (the active ``STDN`` path uses the in-file ``gcn``).
* Removed hardcoded ``.cuda()`` calls; tensors created internally now follow
  the input tensor's device.
* ``STDN.forward`` casts the time index with ``.to(torch.long)`` on the input
  device instead of ``torch.tensor(...)`` (which warned and pinned to CPU).

The class is otherwise structurally unmodified.
"""
...
class TEmbedding(nn.Module):
    ...
    def __init__(self, input_dim, D, num_nodes, bn_decay) -> None:
        ...
        self.FC = FC(input_dims=[input_dim, D, D], units=[D, D, D],
                     activations=[F.relu, F.relu, F.sigmoid], bn_decay=bn_decay)

    def forward(self, X, SE, T, num_vertex, num_his):
        dayofweek = torch.empty(X.shape[0], X.shape[1], 7, device=X.device)
        timeofday = torch.empty(X.shape[0], X.shape[1], T, device=X.device)
        ...
        X_in = torch.cat((timeofday, dayofweek), dim=-1)  # (B, num_his+num_pred, 7+T)
        X_in = X_in.unsqueeze(dim=2)
        X_in = X_in.to(X.device)
...
class AttentionDecoder(nn.Module):
    ...
    X / Y / TE: [batch_size, num_step, num_vertex, D]
...
class GRUEncoder(nn.Module):
    ...
    def forward(self, x):
        B, T, N, Fdim = x.shape
        hidden_state = torch.zeros([B, N, Fdim]).to(x.device)
        output = []
        for i in range(T):
            gx = x[:, i, :, :]
            gh = hidden_state
