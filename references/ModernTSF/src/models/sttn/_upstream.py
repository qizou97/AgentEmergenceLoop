...
class linear(nn.Module):
    def __init__(self, c_in, c_out):
        ...
        self.mlp = torch.nn.Conv2d(
            c_in, c_out, kernel_size=(1, 1), padding=(0, 0),
...
class TemporalAttention(nn.Module):
    def __init__(self, dim, heads=8, window_size=1, dropout=0.,
                 causal=True, stage=0, device=None):
        super().__init__()
        assert dim % heads == 0
        self.dim = dim
...
class SpatialAttention(nn.Module):
    def __init__(self, dim, heads=8, dropout=0., stage=0):
        super().__init__()
        assert dim % heads == 0
        self.dim = dim
        self.num_heads = heads
...
class TemporalTransformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, time_num, dropout,
                 window_size, device):
        super().__init__()
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x = x + self.pos_embedding
        for attn, ff, gcn_layer in self.layers:
            residual = x.reshape(b, t, n, c)
            x = attn(x, adj) + x
            x = ff(x) + x
            x = gcn_layer(residual.permute(0, 3, 2, 1), adj).permute(
...
class STTN(nn.Module):
    ...
    def __init__(self, supports, node_num, input_dim, output_dim, seq_len,
                 horizon, hidden_channels=64, end_channels=128, blocks=3,
                 mlp_expand=4, dropout=0.1, device=None):
        ...
        self.start_conv = nn.Conv2d(
            in_channels=input_dim, out_channels=hidden_channels,
            kernel_size=(1, 1))

        for b in range(blocks):
            self.t_modules.append(
                TemporalTransformer(
                    dim=hidden_channels, depth=1, heads=4,
                    mlp_dim=hidden_channels * mlp_expand,
                    time_num=seq_len, dropout=dropout,
                    window_size=seq_len, device=device))
            self.s_modules.append(
                SpatialTransformer(
                    dim=hidden_channels, depth=1, heads=4,
                    mlp_dim=hidden_channels * mlp_expand,
                    node_num=node_num, dropout=dropout, stage=b))
            self.bn.append(nn.BatchNorm2d(hidden_channels))

        self.end_conv_1 = nn.Conv2d(
            in_channels=hidden_channels, out_channels=end_channels,
            kernel_size=(1, 1), bias=True)
        self.end_conv_2 = nn.Conv2d(
            in_channels=end_channels, out_channels=output_dim * horizon,
            kernel_size=(1, 1), bias=True)

    def forward(self, inputs, label=None):
        """Forward pass. inputs: (B, T, N, F) -> (B, output_dim*horizon, N, 1)"""
        x = inputs.transpose(1, 3)  # (B, F, N, T)
        x = self.start_conv(x)
